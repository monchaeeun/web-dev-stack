/*
    JOIN
    - 2개 이상의 테이블에서 조회하고자 할 때 사용되는 구문
    ...
    => 즉, 관계형 데이터베이스에서 SQL문을 이용한 테이블간에 "관계"를 맺어 
                                            원하는 데이터를 조회하는 방법
                                            
   "오라클 구문(예전 버전)" 과 "ANSI 구문"
   ANSI(미국국립표준협회 == 산업표준을 제정하는 단체)
*/
/*
    등가 JOIN( EQUAL JOIN) / 내부 JOIN (INNER / NATURAL JOIN)
    ...
    -JOIN절에서 같이 조회하고자 하는 테이블을 기술 후 
    매칭시킬 컬럼에 대한 조건을 기술(USING 또는 ON)
    ->연결에 사용하는 컬럼명이 같은 경우 USING(컬럼명)   
*/
-- EMPLOYE, DEPARTMENT 조인해서 사번(EMP_ID), 사원명(EMP_CODE), 부서명(DEPT_NAME)조회
...
SELECT EMP_ID "ID", EMP_NAME"이름", DEPT_CODE"사원명",DEPT_TITLE "부서명"
FROM EMPLOYEE E
