...
-- 컬럼 명이 같은 경우는 USING 사용 추천!
SELECT EMP_ID, EMP_NAME, JOB_CODE,JOB_NAME
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE);

--자연 조인 (NATIRAL JOIN) : 각 테이블마다 동일한 컬럼이 한 개만 존재할 경우
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
...
SELECT * FROM EMPLOYEE;
--DEPARTMENT, LOCATION 참고해서
--전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회

SELECT  DEPT_ID,DEPT_TITLE,L.NATIONAL_CODE,L.LOCAL_NAME
FROM DEPARTMENT D
...
/*
    ...
    - 두 테이블간의 JOIN시 일치하지 않는 행도 포함시켜서 조회가 가능
...
/*
    자체 조인(SELF JOIN)
    -같은 테이블을 다시한번 조인하는 경우(자기 자신과 조인)->대댓글 기능
*/
--사원사번(EMP_ID), 사원명(EMP_NAME), 사원부서코드(DEPT_CODE),사수사번(MANAGER_ID),사수명(EMP_ID와 일치하는 사람),사수부서코드(DEPT_CODE) 조회
SELECT * FROM EMPLOYEE;
...
*/
--사원명, 부서명,직급명 조회
--EMPLOYEE(DEPT_CODE,JOB_CODE),DEPARTMENT(DEPT_ID),JOB(JOB_CODE)
SELECT * FROM EMPLOYEE;
SELECT * FROM DEPARTMENT;
SELECT * FROM  JOB;
...
--국가명(NATIONAL_NAME), 지역명(LACAL_NAME),직급명(JOB_NAME),부서명(DEPT_TITLE),사원명(EMP_NAME),급여(SALARY),급여등급(SAL_LEVEL) 조회
--NATIONAL(NATIONAL_CODE), LOCATION(LOCAL_CODE),JOB(JOB_CODE), DEPARTMENT(DEPT_ID,LOCATION_ID),EMPLOYEE(JOB_CODE),SAL_GRADE(MIN_SAL, MAX_SAL);
...
SELECT N.NATIONAL_NAME"국가명", L.LOCAL_NAME"지역명", J.JOB_NAME"직급명", D.DEPT_TITLE"부서명", E.EMP_NAME"사원명", E.SALARY"급여", S.SAL_LEVEL"급여등급"
FROM EMPLOYEE E
...
--EMPLOYEE(JOB_CODE), LOCATION(LOCAL_CODE), DEPARTMENT(LOCATION_ID) /
--사번(EMP_ID), 직원명(EMP_NAME), 직급명(JOB_NAME), 부서명(DEPT_TITLE), 근무지역(LOCAL_NAME), 급여(SALARY) 조회
SELECT * FROM EMPLOYEE;
SELECT * FROM JOB;
SELECT * FROM LOCATION;
SELECT * FROM DEPARTMENT;

SELECT E.EMP_ID, E.EMP_NAME, J.JOB_CODE, J.JOB_NAME, D.DEPT_TITLE, L.LOCAL_NAME, E.SALARY
FROM EMPLOYEE E
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
...
-- 2. 70(1970)년대생 이면서 여자이고, 성이 전씨인 직원들의 직원명, 주민번호, 부서명, 직급명 조회
SELECT * FROM EMPLOYEE;
SELECT * FROM DEPARTMENT;
SELECT * FROM JOB;
...
JOIN JOB USING(JOB_CODE)
WHERE SUBSTR(EMP_NO,INSTR(EMP_NO,'-')-6,2) BETWEEN 70 AND 79 AND SUBSTR(EMP_NO,INSTR(EMP_NO,'-')+1,1) = 2 AND EMP_NAME LIKE '전%';

-- 3. 보너스를 받는 직원들의 직원명(EMP_NAME), 보너스(BONUS), 연봉(SALARY * 12), 부서명(DEPT_TITLE), 근무지역(LOCATION_NAME) 조회 (단, 부서 배치 안된 사원들도 출력)
SELECT * FROM EMPLOYEE;
...
-- 4. 각 부서별 평균 급여를 조회하여(AVG) 부서명, 평균 급여 조회 (단, 부서 배치 안된 사원들의 평균도 같이)
SELECT * FROM EMPLOYEE;
SELECT * FROM LOCATION;--LOCAL_CODE
SELECT * FROM DEPARTMENT;--LOCATION_ID
SELECT * FROM JOB;--LOCATION_ID


SELECT nvl(DEPT_TITLE, '부서없음') "부서",to_char(FLOOR(AVG(SALARY)), '999,999,999')"평균 급여"
FROM employee
left join department on(dept_code = dept_id)
GROUP BY DEPT_TITLE;
...
SELECT 
DEPT_TITLE "부서명", SUM(SALARY)"급여 합"
FROM EMPLOYEE
...
SELECT EMP_ID,
       ...
       JOB_NAME,
       SAL_LEVEL,
        DECODE(SAL_LEVEL,'S1','고급','S2','고급','S3','중급','S4','중급','S5','초급','S6','초급')"등급"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL);
...
-- 16. MBTI별 평균 평점 조회
SELECT MBTI, ROUND(AVG(RATING),1)"평점 조회"
FROM USER_INFO
...
SELECT TITLE, COUNT("COMMENT")"리뷰 수",AVG(RATING)"평점"
FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE
...
--MOVIE_ID<->MOVIE
--ACTOR_ID<->ACTOR
SELECT NAME , COUNT(MOVIE_ID)"출연 영화수"
FROM MOVIE_ACTOR
JOIN MOVIE USING(MOVIE_ID)
JOIN ACTOR USING(ACTOR_ID)
GROUP BY NAME
