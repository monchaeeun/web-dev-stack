...
/*
        1-2. 컬럼 수정 (MODIFY)
            - 데이터 타입 변경 : MODIFY 컬럼멸 변경할 데이터타입;
            -기본값 변경 : MODIFY 컬럼명 DEFAULT 변경할 기본값;
*/
--변경하려는 자료형의 크기보다 이미 큰 값이 존재해서 에러!
ALTER TABLE USER_INFO MODIFY ADDRESS VARCHAR2(20);
...
ALTER TABLE USER_INFO
    ...
    MODIFY LNAME DEFAULT '아시아';--이미 값이 있음
...
--부모키(PRIMARY KRY) 참조되고있는 컬럼이 있다면 삭제 불가능!
ALTER TABLE BOOK DROP COLUMN BK_NO;
--참조되고있는 컬럼이 삭제되어 있을시 가능!
ALTER TABLE PUBLISHER DROP COLUMN PUB_NO;
/*
    1-4. 컬럼명 변경
    RENAME COLUMN 기존 컬럼 명 TO 변경할 컬럼 명;
*/
...
*/
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C008453;
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_PK;

/*
    2-3. 제약조건명 변경
    RENAME CONSTRAINT 기존 제걍조건명 TO 변경할 제약조건명;
    
*/

ALTER TABLE DEPT_COPY
RENAME CONSTRAINT SYS_C008455 TO DEPT_ID_PK;
