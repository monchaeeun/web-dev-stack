...
/*
    ...
    INSERT ALL 
    INTO 테이블명 1(컬럼,...) VALUES(값, ...)
    INTO 테이블명 2(컬럼,...) VALUES (값, ...)
    서브쿼리;
    
    INSERT ALL 
    WHEN 조건 1 THEN 
        INTO 테이블명1 (컬럼, ...)VALUES(값1,...)
        WHEN 조건 2 THEN 
        INTO 테이블명2 (컬럼, ...)VALUES(값1,...)
...
FROM EMPLOYEE
WHERE 1=0;
--EMPLOYEE 에서 부서 코드가 D1인 직원의 사번, 이름, 부서코드, 입사일

TRUNCATE TABLE EMP_DEPT;
TRUNCATE TABLE EMP_MANEGER;
INSERT ALL 
...
/*
    ...
    -SET 절에서 여러 개의 컬럼을 콤마(,)로 나열해서 
                        값을 동시에 변경할 수 있다.
                        
    - WHERE 절을 생략하면 모든행의 데이터가 변경된다.
*/
SELECT * FROM USER_02;
...
--사실 조건에서는 PRIMARY KEY 조건을 거는걸 전제로 한다.
...
--EMPLOYEE에서 EMP_NAME 이 선동일인 사람의 급여를 7000원, 보너스를 0.2로 변경
UPDATE EMPLOYEE
SET SALARY = 7000,
    BONUS = 0.2
WHERE EMP_NAME = '선동일';
