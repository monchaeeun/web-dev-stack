...
BEGIN
    ...
    DPMS_OUT.PUT_LINE('이름 : ' || EMP.EMP_NAME);
    DPMS_OUT.PUT_LINE('입사일 : ' || EMP.HIRE_DATE);
...
--단, 보너스를 받지않는 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다,' 출력
...
-- 출력은 '당신의 점수는 95점이고, 학점은 A 학점 입니다.'
DECLARE
SCORE NUMBER;
GRADE CHAR(1);
BEGIN
...
/
/*
테이블을 EMPLOYEE->SALARY
500만원 이상이면 '고급', 300만원 이상이면'중급' 300만원 미만이면 '초급'

사원의 사번을 입력받아서 '해당 사원의 급여 등급은|| SAL ||입니다'
*/
...
IF EMP.SALARY >= 5000000 THEN DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 고급 입니다');
ELSIF EMP.SALARY >= 3000000 THEN DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 중급 입니다');
ELSE DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 초급 입니다');
END IF;
END;
/
...
/*
    ...
    END CASE;    
*/
-- 사번을 입력받은 후 부서 코드에 따라 
--D1인 경우는 개발팀, D2인 경우는 디자인팀, D3인 경우는 기획팀
--그 외에는 서비스 팀
--출력 : '해당 사원 '|| OO|| ' 은' || DEPT || ' 입니다.'
DECLARE 
DEPT EMPLOYEE%ROWTYPE;
TEAM VARCHAR2(30);
BEGIN
SELECT *
INTO DEPT
...
ELSE TEAM := '서비스팀';
END CASE;
DBMS_OUTPUT.PUT_LINE(DEPT.EMP_NAME|| '은 ' || TEAM ||' 입니다.');
END;
/
...
-- FOR 문  
 ...
 /*
    ...
    EXCEPTION 
        WHEN 예외면1 THEN 예외 처리 구문1;
        WHEN 예외면2 THEN 예외 처리 구문2;
        ...
    * 오라클에서 미리 정의되어 있는 시스템 예외
    - NO_DATA_FOUND : SELECT한 결과가 한 행도 없는 경우
    - TOO_MANY_ROWS : SELECT 한 결과가 한 행이 리턴되어야 하는데 여러 행인 경우
...
--EMPLOYEE에 부서 코드(DEPT_CODE)를 입력받아서
--사원을 조회하려고 할 때
--NO_DATA_FOUND : 조회 결과가 없습니다
--TO_MANY_ROWS : 너무 많은 행이 조회되었습니다.
