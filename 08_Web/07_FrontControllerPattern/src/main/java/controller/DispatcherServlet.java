...
public class DispatcherServlet extends HttpServlet {
	...
	/*
	 * Controller에서 Servlet을 하나로 만들자! Fromt Controller Pattern
	 * 
	 * 소프트웨어 디자인 패턴 중 하나로 웹 애플리케이션 등에서 사용되는 패턴 중 하나 애플리케이션의 진입점은 중앙집중화하여 요청을 처리하고ㅡ
	 * 전채적인 흐름을 제어하는 역할
	 * 
	 ...
	 * 핸들러(컨트롤러)로 라우팅하고, 해당 컨트롤러의 실행 및 응답 생성등을 관리
	 * 
	 */

	protected void service(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 요청이 어디에서 들어온 요청이지 값을 하나 더 받음 - command
	...
	// 로그아웃
	protected String logout(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException {
		HttpSession session = request.getSession();
