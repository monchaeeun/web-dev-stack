...
## 웹 서버 (Web Server)
    ...
    이미지를 HTTP 프로토콜을 톨해 웹 브라우저에 제공하는 서버

    - 서버 내부의 이미 만들어져 있는 정적인 요소들을 화면에 제공하는 역할

    - Apache : Apache Software Foundation 에서 만든 웹 서버로 HTTP
...
## 웹 애플리케이션 서버(WAS, Wep Application Server)
    ...
    - Topcat : Apache Software Foundation 에서 Servlet과 JSP를 통한
    동적인 웹 문서를 처리하기 위해 만든 웹 애플리케이션 서버

## 서블릿(Servlet)

     - 웹 서비스를 위한 자바 클래스를 말하며 자바를 이용해서 웹을 만들기 위한기술

     - 웹 브라우저로부터 요청을 받아 처리하고 결과르 다시 웹 부라우저로 응답하는 역할

## 동작구조

    1. 클라이언트가 웹 브라우저를 통해 요청을 보낸다.
    2. 해당 요처이 서블릿에 대한 요청이면 컨테이너에게 처리를 넘긴다.
    3. 서블릿에서 요청에 대한 실행이 완료된 후에는 컨테인가 실행 결과를 웹서버에 전달한다.
...
## 배포 서술자(Deployment Descriptor)
    ...
    - 웹 애플리케이션 폴더의 WEB-INF 폴더에 web.wml 파일이 배포 서술자다.
