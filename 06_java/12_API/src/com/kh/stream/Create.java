 ...
 *  자식 인터 페이스들이 상속 관계를 이루고 있다.
 *  
 *  Stream APl
 *  - 스트림은 자바 8 부터 추가된 기능으로 컬렉션(배열)의 저장 요소들을 하나씩 참조해서 
 *  람다식으로 처리할 수 있도록 해주는 반복자
 *  - 중간처리(intermediate), 최종 처리(terminal) 작업을 수행
 *  	- 중간 처리 :반복 (peek), 매칭(타입 변환), 필터링, 정렬 등
 *  	- 최종 처리 : 반복(forEach), 카운팅, 평균, 총합등의 집계 처리
 *  
...
public class Create {
	...
	public void method1() 
	{
		// range : 첫번째 파라미터값 ~ 두번째 파라미터값 이전까지의 값을 요소로
		//			가지는 스트림 객체 생성
		IntStream stream = IntStream.range(1, 10);//1 ~ 9
		int sum = stream.sum();
		System.out.println("range의 sum : "+sum);
		//rangeClosed : 첫번째 파라미터값 ~ 두번째 파라미터값 까지의 값을 요소로
		//				가지는 스트림 객체 생성
