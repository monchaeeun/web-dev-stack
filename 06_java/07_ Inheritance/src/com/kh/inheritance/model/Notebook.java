 ...
 * 3. 코드 중복을 제거하여 프로그램의 생산성과 유지 보수에 크게 기여
 * 
 *  - 상속의 특징
 *  1. 클래스 간의 상속은 단일 상속만 가능
 *  2. 명시되지 않아도 모든 클래스는 Object 클래스 상속
 *  	-> Object 클래스의 메서드들을 오버라이딩 하여 메서드 재정의 가능
 *  3. 부모 클래스의 생성자, 초기화 블록은 상속되지 않는다.
 *  	-> 자식 클래스 생성 시 부모 클래스 생성자가 먼저 실행
 *  4. 부모의 private 변수는 상속은 되지만 직접 접근은 불가능
...
public class Notebook extends Product{
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {
		...
		super.name = name;// 부모가 protected - 상속관계일 때 자식 객체엥서 접근 가능
