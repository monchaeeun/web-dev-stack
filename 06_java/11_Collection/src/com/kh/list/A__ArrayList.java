 ...
 * 컬렉션의 장점
 * - 저장하는 크기의 제약이 없다.
 * - 데이터의 추가, 삭제, 정렬등의 기능 처리가 간단하게 해결된다.
 * - 여러 타입의 데이터가 저장 가능하다. (제네릭 사용 -> 같은 타입만 들어가도록 함)
 * 
 * List
 * - 자료들을 순차적으로 늘어놓는 구조
 * - 저장되는 객체를 인덱스로 관리하기 때문에 인덱스로 객체를 검색, 삭제할 수 있는 기능
 * - 중복되는 객체 저장 가능하고ㅡ null 값도 저장 가능
 * 
 * List 의 특징
 * - 중복 허용
 * - 순서 있음
...
//		list.addAll(subList);
		...
		System.out.println("평균 연령 : " + total);
		
		//저장된 사람들의 이름만 출력
		for(int i = 0; i < list.size(); i++)
		{
			System.out.println("인덱스" + i +"번째 이름 : " + list.get(i).getName());
		}
		
		// 나이 순서대로 정렬(Comparable 가 있어야 실행됨)
		
		//이름 순으로 정렬(this.name.compareTo(o.name))
	...
	public void method3() {
		...
		// 10. 오름차순 정렬 : Comparable 인터페이스를 구현하고 있는 
		//					요소를 가지고 비교값들 반환하여 정렬
		Collections.sort(list);
	...
	{
		A__ArrayList a = new A__ArrayList();
//		a.method1();
		a.method2();
//		a.method3();
	}
