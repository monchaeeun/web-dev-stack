...
public class Car 
{
	 ...
	 *  - 클래스, 변수, 메서드, 생성자에 사용되어 외부로부터 접근제한
	 *   
	 *  + public : 접근 제한이 전혀 없음
	 ...
	 * - 반환형이 존재하지 않음 -> 만약 return이 있으면 일반 메서드로 인식
	 * 
	 * *기본 생성자(default constructor)
	 * - 매개변수가 없는 생성자
	 * - 클래스에 생성자가 하나도 없으면 컴파일러가 기본 생성자를 자동으로 추가
	 * - 생성자가 하나라도 있으면 컴파일러는 기본 생성자를 추가하지않음.
	*/
	...
	//오버라이딩(toString) : 부모 클래스의 메서드를 자식 클래스에서 다시 재정의
	@Override
